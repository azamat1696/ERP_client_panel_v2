<template>
  <div class="q-pa-md">
    <q-table
      title="Ofis Noktaları"
      :rows="citiesRows"
      :columns="baseSetting.stateColumns"
       row-key="id"
      :filter="baseSetting.filter"
    >
      <template v-slot:top-right>
                <q-input outlined dense debounce="300" color="primary" v-model="baseSetting.filter"  placeholder="Şehir Arayınız..."  >
      ...
      <template v-slot:body="props">
        <q-tr :props="props" :key="props.row.index" class="text-center">
          <q-td>
            {{ {...this.$store.getters['CountryModule/countryById'](props.row.country_id)}.CountryName }}
          </q-td>
          <q-td>
            {{ props.row.CityName }}
          </q-td>
          ...
          >
            ...
            <div class="row">
              <div class="col-4 q-pa-xs">
                <div class="text-subtitle2 text-grey-8"> Ülke Seçiniz </div>
                <q-select
                  v-model="formFields.country_id"
                  :options="countries"
                  :option-label="(items) => items.CountryName"
                  :option-value="(items) => items.id"
                  :lazy-rules="true"
                  :rules="[ val => val && val.CountryName.length > 0 || 'Zorunlu alan']"
                  outlined
                  dense
                  hide-bottom-space
                />
...
<script>
import  stateColumns from './CiteisColumuns'
import { ref } from 'vue'
...
export default {
  ...
  methods : {
    onSubmit() {
      ...
      formData.append('country_id',this.formFields.country_id.id);
      formData.append('Status',this.formFields.Status);
      formData.append('Positions',JSON.stringify(this.center));
       this.$store.dispatch('Cities/create',formData).then(res => {
        this.onReset()
    ...
    onReset() {
      ...
      this.formFields.country_id='';

    },
    onEdit(id) {
       const record = this.citiesRows.find(e => +e.id === +id)
       ...
       this.formFields.country_id =  this.$store.getters['CountryModule/countryById'](+record.country_id)
    ...
    onUpdate(){
      ...
      this.$store.dispatch('Cities/update',formData).then(res => {
         this.onReset()
         this.baseSetting.showFormDialog = false
      })
    }
