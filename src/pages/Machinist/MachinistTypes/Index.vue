<template>
<div class="q-pa-md">
  <q-table
    title="Makinist Türleri"
     :rows="machinists"
    :columns="settingsFields.machinistColumns "
    row-key="id"
    :filter="filterMachinistTypes"
  >
    <template v-slot:top-right>
      <q-input outlined dense debounce="300" color="primary" v-model="filterMachinistTypes"  placeholder="Arayınız..."  >
        <template v-slot:append>
          <q-icon name="search" />
        </template>
      </q-input>
      <q-btn
        color="primary"
        label="Yeni Kayıt "
        icon="add"
        class="q-mr-xs q-ml-xs q-pa-sm q-pr-md"
        @click=" settingsFields.showFormDialog = true"
      />
      <q-btn
        color="blue-grey-8"
        icon="archive"
        label="Excel Oluştur"
        class="q-pa-sm"
      />
    </template>

    <template v-slot:body="props">
      <q-tr :props="props" :key="props.row.index" class="text-center">
        <q-td>
          {{ props.row.TypeName }}
        </q-td>
        <q-td>
          <q-chip  square :color="+props.row.Status ===1 ? 'positive' :'negative'" text-color="white" :icon="+props.row.Status ===1 ?'done' : 'clear'" size="13px">
            {{ +props.row.Status === 1 ? 'Açık' :'Kapalı' }}
          </q-chip>
        </q-td>
        <q-td>
          {{ settingsFields.dateFormat(props.row.created_at) }}
        </q-td>
        <q-td class="text-right">
          <q-btn flat round color="blue-grey-9"  @click="onEdit(props.row.id)" ><i class="fas fa-edit"></i></q-btn>

          <q-btn flat round color="primary"  @click="onRemove(props.row.id)" disable><i class="fas fa-trash-alt"></i></q-btn>
        </q-td>
      </q-tr>

    </template>

  </q-table>

  <q-dialog   v-model="settingsFields.showFormDialog" persistent >
    <q-card style="min-width: 500px">
      <q-bar class="q-pa-md" style="height: 50px">
        <div class="text-h4">Yeni Makinist Tipi</div>

        <q-space />
        <q-btn dense flat icon="close" @click="onReset" v-close-popup>
          <q-tooltip>Kapat</q-tooltip>
        </q-btn>
      </q-bar>

      <q-card-section>
        <q-form
          @submit="onSubmit"
          @reset="onReset"
          class="q-gutter-md"
        >
          <q-input
            outlined
            dense
            v-model="formFields.TypeName"
            label="Makinist Tipi"
            lazy-rules
            :rules="[ val => val && val.length > 0 || 'Zorunlu alan']"
          />


          <q-toggle v-model="formFields.Status" label="Status" color="green"/>

          <div class="text-right" v-if="!formFields.id">

            <q-btn label="Sıfırla" type="reset" color="primary" flat class="q-ml-sm" />
            <q-btn label="Kaydet" type="submit" color="positive"  icon-right="save"  flat/>
          </div>
          <div class="text-right" v-else>
            <q-btn label="Güncelle" type="button" color="blue"  icon-right="save"  flat @click="onUpdate"/>
          </div>
        </q-form>
      </q-card-section>

    </q-card>
  </q-dialog>
</div>
</template>

<script>
import {ref} from "vue";
import machinistColumns  from "pages/Machinist/MachinistTypes/MachinistGroupColumns";
import {dateFormat} from "pages/Js/Helpers";
import {Dialog} from "quasar";

export default {
  name: "MachinistTypes",
  data(){
    return {
      filterMachinistTypes : ref(''),
       settingsFields : {
        showFormDialog : ref(false),
        machinistColumns ,
        dateFormat,

      },
      formFields : {
        TypeName : '',
        Status : ref(true),
        id : ''
      }
    }
  },
  methods : {
    onSubmit(){
       let formData = new FormData();
       formData.append('TypeName',this.formFields.TypeName)
       formData.append('Status',this.formFields.Status)
       this.$store.dispatch('MachinistTypesModule/create',formData).then( (res) => {
         this.onReset();
         this.settingsFields.showFormDialog = false
       })
    },
    onReset(){
       this.formFields.TypeName = '',
       this.formFields.Status = true,
       this.formFields.id = ''
    },
    onEdit(id){

      const machinist = this.machinists.find(el => {
            return parseInt(el.id) === parseInt(id)
      })
      this.formFields.TypeName = machinist.TypeName,
      this.formFields.Status = +machinist.Status === 1,
      this.formFields.id = machinist.id
      this.settingsFields.showFormDialog = true

    },
    onUpdate(){
      let formData = new FormData();
      formData.append('TypeName',this.formFields.TypeName)
      formData.append('Status',this.formFields.Status)
      formData.append('id',this.formFields.id)
      formData.append('_method','PUT');

      this.$store.dispatch('MachinistTypesModule/update',formData).then(()=> {
        this.settingsFields.showFormDialog = false;
        this.onReset();
      })
    },
    onRemove(id){
          Dialog.create({
            title: 'Eminmisiniz ?',
            message: 'Silmeniz durumunda işlemi geriye alamzsınız.!',
            persistent: true,
            ok:{
              label: 'Sil',
              flat: true
            },
            cancel: {
              label:'İptal Et',
              flat: true
            }
          }).onOk(()=> {
             this.$store.dispatch('MachinistTypesModule/destroy',id);
          }).onCancel(()=>{
           })
    }
  },
  mounted() {
    this.$store.dispatch('MachinistTypesModule/get')
  },
  computed: {
    machinists(){
       return this.$store.getters['MachinistTypesModule/machinistTypes']
     }
 }
}
</script>

<style scoped>

</style>
