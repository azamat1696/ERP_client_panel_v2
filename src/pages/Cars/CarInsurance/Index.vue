<template>
  <div class="  q-pa-md">
    ...
    >
      ...
      <template v-slot:body="props">
        <q-tr :props="props" :key="props.row.index" class="text-center">
          ...
          <q-td>
          Sigorta Tütü:  <b>{{ props.row.Type }}</b>
          </q-td>
          <q-td>
          ...
          <q-td>
            <b>{{ props.row.InsurancePrice }} TL</b>
          </q-td>
          <q-td>
            {{ props.row.Note }}
          </q-td>
          <q-td class="text-center">
            <q-btn icon="cloud_download"  color="blue-grey-8" fab flat  @click="this.$store.dispatch('CarInsuranceModule/getCarInsuranceDownload',props.row.id)" >
            ...
            <q-btn flat round color="primary"><i class="fas fa-trash-alt" disabled></i></q-btn>
          </q-td>
        </q-tr>
    ...
    <q-dialog
      v-model="editInsuranceDialog"
      full-width
      persistent
    >
      <q-card style="width: 650px !important;" class="q-pb-sm">
        <q-bar style="height: 50px" class="blue-grey-8">
          <div>Zorunlu Sigorta Kaydı Düzenle</div>
          <q-space />
          <q-btn dense flat icon="close" v-close-popup @click="onReset">
            <q-tooltip>Kapat</q-tooltip>
          </q-btn>
          ...
          <q-card-section class="q-pt-none" v-show="editSelectedCarShow">
            ...
            <div class="row q-pa-xs q-pb-none">
              ...
              <div class="col-2 text-right q-pa-none q-ma-none flex-center">
                <q-btn icon="print" color="blue-grey-8 secondary text-right" dense  label="Yazdır" flat  @click="this.$store.dispatch('CarInsuranceModule/getCarInsuranceDownload',formFields.id)"
            ...
            <div class="row">
              <div class="q-pa-xs q-mt-sm">
                <q-banner dense class="bg-grey-1 q-pa-sm  ">
                  ...
                  <div class="text-subtitle2 text-bold">Bilgilendirme</div>
                  <div class="text-weight-regular">
                    Aracın Zorunlu Sigorta raporunu tarih aralığına göre alabilirsiniz tarih aralığı seçmezseniz hepsini getirecektir.
                  </div>
...
const InsuranceColumns = [
  ...
  { name: 'Type', label: 'Sigorta Tipi', field: 'Type',align:'center', },
  ...
  { name: 'InsurancePrice', label: 'Ücreti', field: 'InsurancePrice',align:'center', },
...
export default {
  ...
  setup(){
    return {
      ...
      date,
      Constants,
      editInsuranceDialog:ref(false),
      editSelectedCarShow: ref(false),
      addSelectedCarShow: ref(false),
      addInsuranceDialog: ref(false),
  ...
  computed:{
    getCarInsurance(){
      return this.$store.getters['CarInsuranceModule/getCarInsuranceState']
    }
  },
  methods: {
    onSubmit() {
      Dialog.create({
        ...
        message:  this.carDetails.LicencePlate+' Plakalı aracın mevcut zorunlu sigorta statüsü kapatılarak yerine yeni zorunlu sigorta tarihleri eklenecektir. ',
        persistent: true,
        ok : {
          label : 'Evet Tamamla',
          flat:true
    ...
    onEdit(data) {
      ...
      this.formFields.InsurancePrice = data.InsurancePrice
      this.formFields.Note = data.Note
      this.formFields.InvoiceFile = []
    ...
    onSubmitUpdate(){
      ...
      this.$store.dispatch('CarInsuranceModule/update',formData).then( res => {
